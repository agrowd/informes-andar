...
// hints se obtienen desde API para evitar importar fs en client
...
function FormContent() {
	...
	// Obtener rol después de los hooks
	...
	const addToast = (text: string, type: 'success' | 'error' | 'info' = 'info') => {
		const id = Date.now();
		// Evitar duplicados exactos en menos de 2 segundos
		...
		// Auto-dismiss se maneja en el componente, pero aseguramos limpieza
		setTimeout(() => {
	...
	// Guardado automático en DB con debounce
	useEffect(() => {
		...
		// Serializar datos para comparar
		...
		// Si no tiene datos mínimos, no guardar aún
		...
		// Debounce: guardar en DB después de 2 segundos sin cambios
	...
	useEffect(() => {
		...
		if (selectedYoung) {
			// Cargar TODOS los datos del joven seleccionado (siempre sobrescribir)
			setData((prev: any) => {
				const next = { ...prev };
				if (!next.datosGenerales) next.datosGenerales = {};
				
				// Cargar nombre completo (siempre)
				...
				// Cargar círculo de apoyo (solo si no hay miembros ya cargados, para permitir edición)
				if (selectedYoung.circuloApoyo && Array.isArray(selectedYoung.circuloApoyo) && selectedYoung.circuloApoyo.length > 0) {
					...
					// Solo cargar si no hay miembros ya editados
	...
	// Cargar datos desde DB o desde reportId/formId en URL
	useEffect(() => {
	...
	// Mapeo de paths a campos "Otro" específicos
	...
	const saveOtroModal = () => {
		...
		// Guardar el valor en el path correspondiente
		onChange(otroModal.path, otroModal.value.trim());
		
		// Encontrar el path del array correspondiente
		...
		if (arrayPath) {
			...
			// Agregar "Otro" al array si no está
	...
	const cancelOtroModal = () => {
		// No permitir cancelar sin completar - solo mostrar alerta
		if (!otroModal.value.trim()) {
			alert('Debe completar el campo "Otro" para poder continuar. Este campo es obligatorio.');
			return;
		}
		// Si tiene valor, permitir cancelar (aunque debería usar Guardar)
		// Pero para ser estricto, mejor no permitir cancelar nunca
		alert('Para continuar, debe hacer clic en "Guardar y continuar". El campo "Otro" es obligatorio.');
	};

	// Wizard: orden y navegación según documento institucional
	const steps = [
		'1. Datos generales',
	...
	const canProceed = (s: number) => {
		// No permitir avanzar si hay modal de "Otro" abierto
		if (otroModal.isOpen) {
	...
	const onChange = (path: string, value: any) => {
		// Si no puede editar campos, solo permitir cambios en campos específicos permitidos
		// Los facilitadores pueden agregar notas pero no editar campos principales
		if (!canEditFields) {
			// Permitir solo cambios en notas (Notas) y selección de opciones
			...
			if (!isNoteField && !isOptionSelection && path.startsWith('datosGenerales.') && !isAllowedGeneralField) {
				return; // Bloquear edición de campos principales
			}
		}
		
		// Abrir modal automáticamente cuando se seleccionan opciones que requieren instrucciones
	...
	const validateNow = () => {
		...
		// Reglas condicionales sección 3.x
		const extraErrors: string[] = [];
		
		// Validar "No participó por decisión propia"
		...
		// Validar checkboxes requeridos según el PDF (campos que deben tener al menos una opción)
		...
		Object.entries(requiredCheckboxes).forEach(([path, message]) => {
			...
			if (path === 'evaluacion.dimensiones') {
				// Validar que todas las dimensiones tengan actual y evolucion
		...
		// Bloquear si hay modal de "Otro" abierto
		if (otroModal.isOpen) {
			addToast('Debe completar el campo "Otro" antes de continuar', 'error');
			return false;
		}
	...
	const previewReport = async () => {
		...
		setGenerating(true);
		try {
			// Generar HTML del informe sin crear PDF
	...
	const generate = async () => {
		...
		setGenerating(true);
		try {
			// Guardar formulario primero para obtener formId
		...
		const toggle = (opt: string) => {
			...
			if (isOtro && !values.includes(opt)) {
				...
				if (otroPath) {
					...
					// NO agregar al array todavía, esperar a que complete el modal
					return;
				}
			}
	...
	return (<div>
			...
			<section className="ga-card" style={sectionStyle(7)}>
				...
				<CheckboxGroup label={<EditableText k="sec.7.logros" fallback="Logros" tag="span" />} path="logros" options={logrosOpciones} />
			...
			<section className="ga-card" style={sectionStyle(8)}>
				...
				<CheckboxGroup label={<EditableText k="sec.8.metas" fallback="Metas" tag="span" />} path="suenosMetas.metas" options={metasOpciones} />
				<p className="ga-hint" style={{ marginTop: 8, marginBottom: 8, fontSize: 13, color: '#555' }}>
