...
const xml = fs.readFileSync('scratch/document.xml.txt', 'utf8');
...
for (const rep of replacements) {
  ...
  if (replaced.includes(rep.target)) {
    replaced = replaced.replaceAll(rep.target, rep.repl);
    console.log(`Directly replaced: "${rep.target.substring(0, 30)}..."`);
  } else {
    // Try to see if it's split or has slightly different whitespace
    const normalizedTarget = rep.target.trim().replace(/\s+/g, ' ');
    // We can do a search ignoring XML tags by stripping XML tags for a match, but let's see how many failed.
    console.log(`❌ Failed to find: "${rep.target.substring(0, 30)}..."`);
  }
}
